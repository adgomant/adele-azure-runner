...
Both `outputs.jsonl` (inference) and `judge_outputs.jsonl` (judging) are append-only files. Each attempt is written as a single JSON line immediately after processing, including failed or skipped attempts. This means:
...
At the start of every run, the runner resolves the **latest state per key** by scanning the existing output file. Only keys whose latest row is `status="success"` are treated as already done.
...
2. Resolve the latest row for each `(instance_id, model_id)`
3. Build a set of keys whose latest row has `status="success"`
4. Filter dataset items: skip any where (item.instance_id, model_id) is in that set
...
2. Resolve the latest row for each `(instance_id, model_id, judge_name)`
3. Build a set of keys whose latest row has `status="success"`
4. For each judge: skip instances already successfully evaluated by that judge
...
| Resume after crash | Picks up where it left off, skipping only latest-success rows |
| Re-run after completion | Skips everything (all instances already done) |
| Re-run with `--force-run` | Ignores output dedup and re-executes, but batch judge recovery still runs first |
...
- Do not manually edit `outputs.jsonl` or `judge_outputs.jsonl`. Latest-state resolution depends on valid JSON lines.
