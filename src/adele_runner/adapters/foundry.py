"""Azure AI Foundry request-response adapter."""
...
class FoundryAdapter:
    ...
    async def send(self, request: ChatRequest, *, timeout_s: float) -> ChatResponse:
        ...
        try:
            ...
            logger.error("Foundry request timed out for request_id=%s after %.1fs", request.request_id, timeout_s)
            raise
        except Exception as exc:
            logger.error("Foundry request failed for request_id=%s: %s", request.request_id, exc)
            raise
